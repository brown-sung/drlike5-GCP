...
// --- Gemini API 서비스 ---
async function callGeminiWithSDK(systemPrompt, context, modelName = 'gemini-2.5-flash', isJson = false, timeout = 25000) {
...
async function generateWaitMessage(history) {
    const context = `---대화 기록---\n${history.join('\n')}`;
    try {
        const resultText = await callGeminiWithSDK(SYSTEM_PROMPT_WAIT_MESSAGE, context, 'gemini-2.5-flash', true, 3800);
...
const generateNextQuestion = async (history, extracted_data) => {
    ...
    return await callGeminiWithSDK(SYSTEM_PROMPT_GENERATE_QUESTION, context, 'gemini-2.5-flash');
};

// 종합 분석 함수 (긴 타임아웃)
const analyzeConversation = async (history) => {
    const context = `다음은 분석할 대화록입니다:\n\n${history.join('\n')}`;
    const resultText = await callGeminiWithSDK(SYSTEM_PROMPT_ANALYZE_COMPREHENSIVE, context, 'gemini-2.5-flash', true);
